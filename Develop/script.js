...
function generatePassword() {
  ...
  while (passwordLength < 8 || passwordLength > 128)) {
    passwordLength = prompt("How Long Should Password Be? (Must be between 8-128 characters.");
    ...
    if (shouldIncludeUppercase) {
      options.push(...alphaUpper);
    }
    
    if (shouldIncludeLowercase) {
      options.push(...alphaLower);
    }
    ...
    // randomly pick x amount of chars from options and save it to result;
    var randomLength = function (max, min) {
      return Math.random() * (max - min) + min;
    }
    randomLength(128, 8)
    
    var randomUpper = function () {
      return Math.random(alphaUpper);
    }
    randomUpper(alphaUpper)
    
    var randomLower = function () {
      return Math.random(alphaLower);
    }
    console.log(randomLower(alphaLower))
    ...
    var randomChar = function () {
      return Math.random(specialChar);
    }
